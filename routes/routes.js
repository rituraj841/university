...
exports.registerFormHandler = function(req, res){
   res.render("register-Form.handlebars", {
            AUTHENTICATED:req.session.authenticated,
            IS_ADMIN:req.session.isAdmin,    ////Session
            LOGGED_USER_NAME: req.session.loggedinUser}); // serves new user register form page when clicked from page link in Menu
}; //registerFormHandler
...
exports.registerUserHandler = function(req, res){
   ...
   var newuser = new userModel(); //creating a variable newuser for saving new entries
   ...
   newuser.save(function(err, savedUser){  /////confusion
       if(err){
         ...
         console.log(message);
         res.render("register-form", {errorMessage:message}); //here the message will be populated into {{errorMessage}} in the res.render page 
         return;
       }else{
       ...
       qModel.findOne({qnumber:qn}, function(err, questRec){ //find from qModel in (db.js) qnumber
         if (!err && (questRec !== null) ){
           ...
           res.render('quiz-Exam.handlebars',
            ...
            // answerMessage:req.session.quizmarks,
...
exports.adminDashboardHandler = function(req, res){
   res.render("dashboard-admin.handlebars",      // serves quiz form page when clicked quiz form page link in Menu
          {IS_ADMIN:req.session.isAdmin,
            AUTHENTICATED:req.session.authenticated,
            USER_ROLL:req.session.rollInSession,
            LOGGED_USER_NAME: req.session.loggedinUser});
